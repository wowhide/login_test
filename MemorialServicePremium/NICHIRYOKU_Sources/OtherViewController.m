...
@interface OtherViewController () {
    ...
    NSString *_menuItemModelChangeKey;
...
static NSString *const MENU_KEY_MODEL_CHANGE_KEY    = @"key_model_change_key";
...
- (void)viewDidLoad
{
    ...
    _menuItemTerms              = @"利用規約";
    _menuItemModelChangeKey     = @"機種変更について";
...
//    [_menuKeyArray  addObject:MENU_KEY_SYUKATU];
    ...
    [_menuItemArray addObject:_menuItemModelChangeKey];
    [_menuKeyArray  addObject:MENU_KEY_MODEL_CHANGE_KEY];
...
- (void)tableView:(UITableView *)tableView willDisplayHeaderView:(UIView *)view forSection:(NSInteger)section {
    [view setTintColor:[UIColor colorWithRed:1.00 green:1.00 blue:1.00 alpha:0]];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([selectedKey compare:MENU_KEY_HOWTO] == NSOrderedSame) {
        ...
        //使い方画面に遷移
        [self.navigationController pushViewController:otherHowToUseViewController animated:NO];
    ...
    if ([selectedKey compare:MENU_KEY_BACKUP] == NSOrderedSame) {
        ...
        //データ引き継ぎ画面に遷移
        [self.navigationController pushViewController:otherDataTransferViewController animated:NO];
    ...
    if ([selectedKey compare:MENU_KEY_POINT] == NSOrderedSame) {
        ...
        [self.navigationController pushViewController:[self getPointViewController] animated:NO];
    ...
    if ([selectedKey compare:MENU_KEY_TERMS] == NSOrderedSame) {
        ...
        //利用規約画面に遷移
        [self.navigationController pushViewController:otherRuleViewController animated:NO];
        
        return;
    }
    
    //機種変更について
    if ([selectedKey compare:MENU_KEY_MODEL_CHANGE_KEY] == NSOrderedSame) {
        //機種変更キーのインスタンスを生成
        DataTransferAboutViewController *dataTransferAboutViewController = [DataTransferAboutViewController new];
        
        //機種変更キー画面に遷移
        [self.navigationController pushViewController:dataTransferAboutViewController animated:NO];
