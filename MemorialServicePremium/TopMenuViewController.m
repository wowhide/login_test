...
//    [self.btn_kakeizu.layer insertSublayer:gradient_kakeizu atIndex:0];
    ...
    self.btn_kakeizu.imageView.contentMode = UIViewContentModeScaleAspectFit;
...
//    [self.navigationController pushViewController:kumotsuViewController animated:YES];
    ...
    //会葬受付のインスタンスを生成
    KaisouViewController *kaisouViewController = [KaisouViewController new];
    
    // Tab bar を非表示
    kaisouViewController.hidesBottomBarWhenPushed = YES;
    
    //会葬受付画面に遷移
    [self.navigationController pushViewController:kaisouViewController animated:YES];
}
