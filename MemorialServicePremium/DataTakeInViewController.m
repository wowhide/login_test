...
//    NSString *datatakein_urlAsString = READ_TRANSFER_DATA;
    NSString *datatakein_urlAsString = @"http://ms-dev.wow-d.net/cooperation/readtranseferdata";
...
//NSLog(@"jsonUserObject:%@",[jsonUserObject description]);
        ...
        NSString *downloaddatekey =[jsonUserObject objectForKey:@"data_key"];
        [userDefaults setObject:downloaddatekey forKey:USER_ID];  // data_keyをUSER_IDキーで保存
//        NSLog(@"useridは%@",[jsonUserObject objectForKey:@"data_key"]);
...
//                NSString *downloadphoto_urlAsString = [NSString stringWithFormat:[DOWNLOAD_PHOTO stringByAppendingString:@"?datakey=%@&filename=%@"], accessKey, deceased.deceased_photo_path];
                NSString *downloadphoto_urlAsString = [NSString stringWithFormat:[@"http://ms-dev.wow-d.net/cooperation/downloadphoto" stringByAppendingString:@"?datakey=%@&filename=%@"], accessKey, deceased.deceased_photo_path];
        ...
        [self.delegate hideDataTakeInViewDownload:YES];
    } else {
        //メインスレッドのインディケーターを閉じるを実行
        [self performSelectorOnMainThread:@selector(closeIndicatorAndErrorMessage:) withObject:@"ダウンロードに失敗しました。\nもう一度ダウンロードして下さい。" waitUntilDone:NO];
    }
